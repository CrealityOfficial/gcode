...
namespace gcode
{
    ...
    {
        if (strLineType.find(";TYPE:WALL-OUTER"))
        {
            return SliceLineType::OuterWall;
        }
        else if (strLineType.find(";TYPE:WALL-INNER"))
        {
            return SliceLineType::InnerWall;
        }
        else if (strLineType.find(";TYPE:SKIN"))
        {
            return SliceLineType::Skin;
        }
        else if (strLineType.find(";TYPE:SUPPORT-INTERFACE"))
        {
            return SliceLineType::SupportInterface;
        }
        else if (strLineType.find(";TYPE:SUPPORT-INFILL"))
        {
            return SliceLineType::SupportInfill;
        }
        else if (strLineType.find(";TYPE:SUPPORT"))
        {
            return SliceLineType::Support;
        }
        else if (strLineType.find(";TYPE:SKIRT"))
        {
            return SliceLineType::SkirtBrim;
        }
        else if (strLineType.find(";TYPE:FILL"))
        {
            return SliceLineType::Infill;
        }
        else if (strLineType.find(";TYPE:PRIME-TOWER"))
        {
            return SliceLineType::PrimeTower;
        }
        else if (strLineType.find(";TYPE:Slow-Flow-Types"))
        {
            return SliceLineType::FlowTravel;
        }
        else if (strLineType.find(";TYPE:Flow-In-Advance-Types"))
        {
    ...
    void GCodeStruct::processLayer(const std::string& layerCode, int layer, std::list<int>& stepIndexMap)
    {
    ...
    {
        //std::vector <GcodeTemperature> m_temperatures;
        if (stepCode.find("M140")
            || stepCode.find("M190")
            || stepCode.find("M104")
            || stepCode.find("M109")
            || stepCode.find("M141")
            || stepCode.find("M191")
            || stepCode.find("EXTRUDER_TEMP")
            || stepCode.find("BED_TEMP")
            )
        {
            ...
            for (const auto& it3 : strs)
            {
                ...
                else if (componentStr.find("EXTRUDER_TEMP"))
                {
                ...
                else if (componentStr.find("BED_TEMP"))
                {
    ...
    void GCodeStruct::checkoutLayerInfo(const std::string& stepCode, int layer)
    {
        ...
        if (stepCode.find("TIME_ELAPSED"))
        {
            
			std::vector<std::string> strs = stringutil::splitString(stepCode, ":");//QStringList strs = stepCode.split(":");
            if (strs.size() >= 2)
            {
				float temp = std::atof(strs[1].c_str()) - tempCurrentTime;//strs[1].toFloat() - tempCurrentTime;
                float templog = 0.0f;
                //if (temp >0)
                //{
    ...
    void GCodeStruct::processStep(const std::string& stepCode, int nIndex, std::list<int>& stepIndexMap)
    {
        checkoutTemperature(stepCode);
        checkoutFan(stepCode);

        if (stepCode.find(";TYPE:") && !stepCode.find(";TYPE:end"))
        {
    ...
    void GCodeStruct::processG01(const std::string& G01Str, int nIndex, std::list<int>& stepIndexMap, bool isG2G3)
    {
        ...
        for (const std::string& it3 : G01Strs)
        {
            ...
            {
				tempEndPos.at(0) = std::atof(componentStr.substr(1).c_str());//componentStr.mid(1).toFloat();
                havaXYZ = true;
            }
    ...
    void GCodeStruct::processG23(const std::string& G23Code, int nIndex, std::list<int>& stepIndexMap)
    {
	...
	void GCodeStruct::parseGCodeInfo(gcode::SliceResult* result)
	{
		...
		if (preStr.find(";machine is belt:true"))
			parseInfo.beltType = 1;
		if (preStr.find("Crealitybelt"))
